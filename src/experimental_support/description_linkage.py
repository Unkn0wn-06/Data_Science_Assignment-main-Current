"""Conservative description cleaning and deterministic canonical linkage."""
...
def clean_description_text(series: pd.Series) -> pd.Series:
    """Normalize case and whitespace without discarding numbers or punctuation."""
...
def clean_raw_to_canonical(raw_path) -> pd.DataFrame:
    """Replay the repository cleaning sequence that created canonical eligibility."""
...
def link_descriptions(raw_path, canonical_listing_ids) -> tuple[pd.Series, dict]:
    """Recover exactly one description for each canonical ID without fuzzy matching."""
